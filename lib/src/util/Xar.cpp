...
std::map<std::string, std::pair<char *, size_t>> ExtractInMemory(std::string file) {
  std::map<std::string, std::pair<char *, size_t>> files;
#ifdef HAVE_LIBXAR
  xar_t x;
  xar_iter_t xi;
  xar_file_t xf;
  ...
  for (xf = xar_file_first(x, xi); xf != nullptr; xf = xar_file_next(xi)) {
    ...
    char *buffer;
    size_t size;
    if (xar_extract_tobuffersz(x, xf, &buffer, &size) != 0) {
      std::cerr << "Cannot extract to buffer" << std::endl;
      free(path);
      continue;
    }

    // Add to list of extracted buffers
    files[std::string(path)] = std::make_pair(buffer, size);
